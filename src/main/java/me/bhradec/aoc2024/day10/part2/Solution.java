...
public class Solution {
    ...
    private long findResultFromInput(String path) throws IOException {
        ...
        return sumTrailRates(map, MAX_WIDTH, MAX_HEIGHT);
    }
    ...
    private int sumTrailRates(int[][] map, int width, int height) {
        int result = 0;

        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                if (map[i][j] == 0) {
                    result += countDistinctTrails(map, j, i, width, height, 0);
                }
            }
        }
    ...
    private int countDistinctTrails(
            int[][] map,
            int x,
            int y,
            int width,
        ...
        return countDistinctTrails(map, x - 1, y, width, height, wantedHeight + 1)
            + countDistinctTrails(map, x, y - 1, width, height, wantedHeight + 1)
            + countDistinctTrails(map, x + 1, y, width, height, wantedHeight + 1)
            + countDistinctTrails(map, x, y + 1, width, height, wantedHeight + 1);
    }
